...
hObjFun( mX, mR, mQ, λ ) = (0.5 .* sum(abs2, ((mR * mX) .- mQ))) + ((0.5 * λ) .* sum(abs2, mX));
∇ObjFun( mX, mRR, mRQ, λ ) = ((mRR * mX) .- mRQ .+ (λ .* mX));

function hObjFun!( mX, mR, mQ, λ, mRX )
    ...
    mRX .-= mQ;
    return 0.5 .* (sum(abs2, mRX) .+ (λ .* sum(abs2, mX)));

end

function ∇ObjFun!( ∇mX, mX, mRR, mRQ, λ, mRRX )
...
# Step II
# Solve \arg \min_X || R X - Q ||_F^2 + λ || X ||_F^2
#       subject to X_ij >= thr
...
runTime = @elapsed for ii ∈ 1:numIter
    ...
    ∇ObjFun!(∇mZ, mZ, mRR, mRQ, λ, mRRX);

    mXPrev .= mX;
    mX .= mZ .- (η .* ∇mZ);
    mX .= max.(mZ .- (η .* ∇mZ), minValThr);
    # vObjVal[ii] = hObjFun(mX, mR, mA, mY, λ);
    vObjVal[ii] = hObjFun!(mX, mR, mQ, λ, mRX);
