...
for jj = 1:numCols
    for ii = 1:numRows
        hueVal = 6 * mHsl(ii, jj, 1);
        lumCoeff = 0;
        
        % For kk = 1 we're dealing with circular distance
        diffVal     = min(abs(vHueVal(1) - hueVal), abs(6 - hueVal));
        lumCoeff    = lumCoeff + (vCoeffValues(1) * mHsl(ii, jj, 2) * max(0, 1 - diffVal));
        for kk = 2:numCoeff
            lumCoeff = lumCoeff + (vCoeffValues(kk) * mHsl(ii, jj, 2) * max(0, 1 - abs(vHueVal(kk) - hueVal)));
        end
