% Mathematics Q2057347
% https://math.stackexchange.com/questions/2057347
...
hObjFun = @(vX) 0.5 * sum((vX - vY) .^ 2) + (paramMu * sum(abs(vX))) + (outOfSetCost * (abs(norm(vX) - 1) > outOfSetThr));
...
cvx_begin('quiet')
    ...
    minimize( 0.5 * sum_square(vX - vY) + (paramMu * norm(vX, 1)) );
    subject to
        norm(vX) <= 1;
cvx_end
