...
% Orthogonal Projection onto the Weighted L2 Norm Ball
% References:
%   1.  aa
% Remarks:
%   1.  sa
...
vC = 10 * randn(numElements, 1);
% vC = zeros(numElements, 1);

mI = eye(numElements);

mW = 10 * randn(numElements, numElements);
mW = (mW.' * mW) + (0.01 * mI);
...
cvx_begin('quiet')
    ...
    minimize( 0.5 * pow_pos(norm(vX - vY, 2), 2) );
    subject to
        quad_form(vX - vC, mW) <= paramD;
cvx_end
...
vX = ProjectWeightedL2Ball(vY, mW, vC, paramD);
